...
### 2026-09-26 — Action-items-first restructure (IA + visual redesign)
- **Product decision:** action items are the primary surface, not meetings. Fathom is meeting-first (list of recordings, open one to find what was decided). But the thing a user actually comes back for after a call is *what they owe and what others owe them* — so the landing page is now a cross-meeting action items inbox, and meetings are the secondary drill-down (the source/evidence for each task). Deliberate departure from Fathom's structure, not a reskin.
- Backend: `GET /action-items` (`backend/app/routers/action_items.py`, replaced the old `/{meeting_id}` stub that returned `[]` and was unused) — one Supabase query using PostgREST FK embedding `select("*, meeting:meetings(id, title, date)")`, sorted newest meeting first.
- Frontend:
  - Top header replaced by a sidebar (`frontend/app/sidebar.tsx`: Action items / Meetings, theme toggle at bottom; collapses to a top bar below `md`). Search skipped — out of scope.
  - `/` is now the inbox (`frontend/app/page.tsx`): dense Linear/Things-style rows — open-circle marker, task, source meeting title · date, right-aligned due date column (mono, tabular), owner initial badge (`Speaker N` → `S0`, `Team` → dashed group icon, null → dashed empty). Group by owner (named people first, then Team, then Unassigned) or by meeting (header links to meeting); owner filter chips with counts. One accent (blue-600), no gradients.
  - Each row links to `/meetings/{id}?segment={source_segment_id}`; detail page reads `?segment=` on first load, scrolls the transcript to that segment and highlights it. Otherwise detail page unchanged (back link now → `/meetings`).
  - Meeting list moved as-is to `/meetings` (`frontend/app/meetings/page.tsx`, git mv, imports adjusted).
- **Tested** (Playwright against local backend + real Supabase data): inbox shows 52 items across 10 meetings; owner groups Bilal 8 / Sarah 8 / Speaker 0 20 / Team 16; by-meeting shows 10 groups; Team filter → 16 rows; clicking a row lands on meeting `0203638e…` with the linked segment ("Bilal is finishing the dashboard by Thursday", 0:15) highlighted; `/meetings` lists 10 meetings; mobile (390px) layout checked; no console errors. `tsc` clean; eslint only the pre-existing `theme-toggle.tsx` warning.
